...
const { verifyPassword }= require('../../services/passwordHashing.services')
const { createJwtToken }= require('../../services/jwtToken.services')
...
exports.handlePostLogin= async (req, res)=>{
    try{
        ...
        const user= await User.findOne({ username: username, role: role, isActive: true}).select('+password')
        if(!user){
